/*
ナンバーズ3の確率を出す！
「総合課題4-1」で作成した課題を利用して、フォームに入力した数値を10000回連続でコンピュータが抽選した場合の「ストレート」「ボックス」「ハズレ」の確率を小数点以下2桁まで表示してください。
『HTML』で下記の入力フォームを作り結果を表示しましょう。
--------------------
[0から9の数値] [0から9の数値] [0から9の数値] ←入力部、フォーカスが外れたら入力チェック（バリデーション）を行ってください
...
当選結果：
ストレート：○○.○○% ←ここに結果を表示してください
ボックス：○○.○○% ←ここに結果を表示してください
ハズレ：○○.○○% ←ここに結果を表示してください
--------------------

「javascript_general_mission_4_4.js」ファイルを作成して解答してください。
*/

// ==================================================
// DOM要素の取得
// ==================================================
const userNumber1Input = document.getElementById("user_number_1");
const userNumber2Input = document.getElementById("user_number_2");
const userNumber3Input = document.getElementById("user_number_3");
...
// 複数の入力要素を配列にまとめて管理しやすくします
const userInput = [userNumber1Input, userNumber2Input, userNumber3Input];
...
function randomDrowing(maxValue) {
  const random = Number(Math.floor(Math.random() * maxValue + 1));
  return random;
}
...
const clickCounter = Number(10000);
let stCounter = Number(0);
let boxCounter = Number(0);
let hazure = Number(0);
...
function thousandDrowing() {
  for (let i = 0; i < clickCounter; i++) {
    drowing();
  }
  resultDisplay1.textContent = ((stCounter / clickCounter) * 100).toFixed(2);
  resultDisplay2.textContent = ((boxCounter / clickCounter) * 100).toFixed(2);
  resultDisplay3.textContent = ((hazure / clickCounter) * 100).toFixed(2);
  reset();
}
// ==================================================
// イベントリスナーの設定
...
checkButton.addEventListener(`click`, thousandDrowing);
